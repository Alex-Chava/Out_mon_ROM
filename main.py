...
mon_ip = []  # Для значений из колонки X
...
for i in range(num_rows):  # Обрабатываем все строки
    ...
    pattern = r'ЖК\s*"[^"]+"'
    # Удаление найденных подстрок из строки
    cleaned_text = re.sub(pattern, '', result).strip()
    # Удаление лишних пробелов и запятых
    cleaned_text = re.sub(r'\s+,', ',', cleaned_text)  # Убирает пробелы перед запятыми
    result = re.sub(r'\s{2,}', ' ', cleaned_text)  # Убирает двойные пробелы
    mon_adr.append(result)  # Если ячейка пуста, добавляем None
    result = df.iloc[i, 10]  # Чтение значения из колонки 10
    if pd.notna(result):
        result = f"http://{result}"
    else:
        result = None
    mon_ip.append(result)  # Если ячейка пуста, добавляем None
...
out_df[5] = mon_ip  # Записываем значения в пятую колонку

# Записываем значения из колонки X в 5-ю колонку (индекс 4)
out_df[6] = mon_npu  # Записываем значения в пятую колонку
